...
(export '(nao ip server-port container-name
              make-nao start-nao-server stop-nao-server stop-all-nao-servers test-server-connection
              scp-get-nao clear-nao-recordings clear-local-recordings
              nao-clear-all))
...
(defmethod test-server-connection ((nao nao)
  ...
  (let* ((message (make-json 'test-connection :data '((test-message . "test-server-connection"))))
         (reponse (curl message :host (server-host nao) :port (server-port nao))))
    (unless silent
      (curl (make-json 'speak :data `((speech . ,(format nil "Connected to Babel 2 at port ~a" (server-port nao)))))
    ...
    (when (assoc :test-message reponse)
      (when (string= (rest (assoc :test-message reponse)) "test-server-connection")
        t))))

;; Getting files from nao
;;;;;;;;;;;;;;;;;;;;;;;;;;

(defgeneric scp-get-nao (nao remote-file local-file &key)
  (:documentation "Gets remote-file from the Nao and stores it in local-file"))

(defmethod scp-get-nao ((nao nao)
                        remote-file
                        local-file
                        &key)
