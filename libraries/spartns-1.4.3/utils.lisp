...
(defmacro fast-binary-search  (the-element vector &key
			       (element-type t)
			       (test= 'eql)
			       (test< '<)
			       (get-element 'aref)
  ...
  (when (null end)
    (setq end `(1- (length ,vector))))
  (jp-utils::with-gensyms (inside-binary-search el vec istart iend middle middle-element)
		...
		(if (< ,iend ,istart)
		    ...
		    (let* ((,middle (floor (the fixnum (+ ,istart ,iend)) 2)) 
		      ...
		      (cond ((,test= ,el ,middle-element) ; found it!
			    ...
			    (t                          ; to the right...
			     (,inside-binary-search ,el ,vec (1+ ,middle) ,iend)))))))
       (flet ((,external-binary-search (,el ,vec &key (,istart 0) (,iend -1))
		(,inside-binary-search ,el ,vec ,istart ,iend)))
