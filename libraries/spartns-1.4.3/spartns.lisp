...
(defscheme  :name     hash
  :type     hash-table
  :make     (make-hash-table :size #+poplog (max 1 size) #-poplog size)
  :get      ;(gethash index data sparse-element)
  	    (multiple-value-bind (v f) (gethash index data)
	      (values (the element-type (if f v sparse-element))
		      (the boolean f)))
  :set      (setf (gethash (the fixnum index) data) value);(the element-type value)) FIXME!
  :delete   (remhash index data)
  :traverse #|oop for index fixnum being the hash-keys of data do
...
(defscheme :name     cvector
  ...
  :delete     (set-cvec data index sparse-element element-type sparse-element (lambda (x)
										(declare (ignore x)) 0))
...
(defmacro defspartn (name &key
		     ...
		     sparse-element
		     (test-equal 'eql)
		     (resize-function '(lambda (x) (declare (ignore x)) 200))
		     (resize-amount   -1)
		     (def t)
		     (declare nil))
