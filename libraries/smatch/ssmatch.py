...
match_triple_dict = {}
# Variables which the matching is more important than the others; give it more weight: 0.5 of the weight for these variables and 0.5 for the rest 
focused_variables = []
weights = []
W = 1

#Our Code: FCG meaning reader
def get_meaning(input_f, prefix):
   """
   Read a FCG meaning and convert it to an AMR format
   """
   instances = []
   attributes = []
   relations = []
   ...
   """ check for variables that are more important """
   m = input_f.strip().split(";")	  
   if len(m) > 1:
	input_f = m[0]
	var = m[1].strip().split(" ")
	for v in var:
		focused_variables.append(v)
   ...
   # Update the variables for the new name
   for i in range(0, len(focused_variables)):
 	if focused_variables[i] in mappings:
        	focused_variables[i] = mappings[focused_variables[i]]

   weights.append(1 - W)
   if len(focused_variables) > 0:
   	weights.append(W)
...
def build_arg_parser():
    ...
    parser.add_argument('-m', nargs=2, required=True, type=str,
                        help='Two meaning networks: \'parsed\' and \'annotated meaning network\'; variables to focus on. This option is required.')
...
def build_arg_parser2():
    ...
    parser.add_option("-m", "--meaning networks", nargs=2, dest="m", type="string",
                      help='Two meaning networks: \'parsed\' and \'annotated meaning network; variables to focus on\'. This option is required.')
    ...
    parser.add_option('--pr', "--precision_recall", action='store_true', dest="pr",
                      help="Output precision and recall as well as the f-score. Default: false")
    parser.set_defaults(r=4, v=False, pr=False)
    return parser
...
def compute_focused_match(mapping, prefix1, prefix2, instance1, instance2, attribute1, attribute2, relation1, relation2):
    """ calculate the match based on the focused variables """
    focused_match = 0
    ...
    for i, m in enumerate(mapping):
        ...
        weight = weights[0]
        if m in focused_variables and len(weights) > 1:
            weight = weights[1]
	...
	for i in range(0, len(instance1)):
            for j in range(0, len(instance2)):
                ...
                if instance1[i][0].lower() == instance2[j][0].lower() \
                   ...
                   node2_index = int(instance2[j][1][len(prefix2):])
                   node_pair = (node1_index, node2_index)
 		   if node_pair == current_node_pair:
		      focused_match += weight;
        ...
        for i in range(0, len(relation1)):
            for j in range(0, len(relation2)):
                ...
                if relation1[i][0].lower() == relation2[j][0].lower():
                   ...
                   node_pair1 = (node1_index_amr1, node1_index_amr2)
                   node_pair2 = (node2_index_amr1, node2_index_amr2)
        	   if node_pair1 == current_node_pair:
		      focused_match += weight; 
		   if node_pair2 == current_node_pair:
		      focused_match += weight; 
...
def compute_match(mapping, weight_dict):
    ...
    for i, m in enumerate(mapping):
		...
		#print weight_dict[current_node_pair][key]
		#print weight * weight_dict[current_node_pair][key]
...
def compute_f(match_num, test_num, gold_num):
    ...
    Args:
        ...
        test_num:  triple number of AMR 1 (test meaning network)
        gold_num:  triple number of AMR 2 (gold meaning network)
    Returns:
...
def calculate_weighted_value(prefix, instances):
    value = 0

    for m in focused_variables:
	var = prefix + str(m)
	for i in instances:
	    if var in i:
	       value = value + weights[1]
            else:
...
def main(arguments):
    ...
    (instance1, attributes1, relation1) = get_meaning(args.m[0], prefix1)
    (instance2, attributes2, relation2) = get_meaning(args.m[1], prefix2)

    if len(args.m[1].strip().split(";")) != 2:
	print >> ERROR_LOG, "Error: annotated network is missing the variables to be focusing on!"
	return

    if instance1 == "" and attributes1 == "" and relation1 == "" and instance2 == "" and attributes2 == "" and relation2 == "":
	print >> ERROR_LOG, "Error: meaning networks missing!"
	return
    ...
    test_triple_num_ = test_triple_num
    gold_triple_num_ = gold_triple_num
    if len(focused_variables) > 0:
	test_triple_num = calculate_weighted_value(prefix1, instance1) + calculate_weighted_value(prefix1, attributes1) + calculate_weighted_value(prefix1, relation1)
	gold_triple_num = calculate_weighted_value(prefix2, instance2) + calculate_weighted_value(prefix2, attributes2) + calculate_weighted_value(prefix2, relation2)
	best_match_num = compute_focused_match(best_mapping, prefix1, prefix2, instance1, instance2, attributes1, attributes2, relation2, relation2)
    ...
    if pr_flag:
	print 'F-score:'+ str(best_f_score) + '\nPrecision:' + str(precision) + '\nRecall:' + str(recall)
    else:
	print 'F-score:'+ str(best_f_score)
...
if __name__ == "__main__":
    ...
    if sys.version_info[1] < 7:
        import optparse
        if len(sys.argv) == 1:
            print >> ERROR_LOG, "No argument given. Please run ssmatch.py -h \
            to see the argument description."
            exit(1)
        parser = build_arg_parser2()
        (args, opts) = parser.parse_args()
        if args.m is None:
            print >> ERROR_LOG, "ssmatch.py requires -m option to indicate two meaning networks \
                                 containing AMR as input. Please run ssmatch.py -h to  \
